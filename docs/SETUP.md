# Setup Guide

> How to set up and configure the AI Workspace for your environment.

---

## Prerequisites

- Git
- Bash or Zsh
- AI tool: Muse Code (Meta), OpenCode (recommended), Claude Code, Gemini CLI, GitHub Copilot, or Cursor

---

## Quick Setup

```bash
# 1. Clone as your workspace
git clone <repo-url> ~/.agentic-harness
cd ~/.agentic-harness

# 2. (Optional) Make it your own — remove upstream history
rm -rf .git && git init && git add . && git commit -m "chore: init workspace"

# 3. Make bin scripts executable
chmod +x ./bin/*

# 4. Initialize project directories
agent-toolkit project init

# 5. Clone your repos
agent-toolkit project clone owner/my-repo

# 6. Open in your AI tool
opencode        # or: claude / cursor / gemini
```

---

## Step-by-Step Installation

### 1. Clone the repository

```bash
git clone <repo-url> ~/.agentic-harness
cd ~/.agentic-harness
```

### 2. Make scripts executable

```bash
chmod +x ./bin/*
```

### 3. Initialize workspace

```bash
agent-toolkit project init
# Creates repos/ and projects/ directories
```

### 4. Clone your repos

```bash
# Clone repos you work with
agent-toolkit project clone owner/my-project
agent-toolkit project clone owner/my-other-project
```

### 5. Start using with AI

```bash
# Recommended
opencode

# Or any of these
claude
gemini
cursor
gh copilot session
```

---

## Configuration Files

### projects.yaml

Auto-generated by `agent-toolkit devcompanion` whenever you queue a job or run
`devcompanion projects`. **Do not edit manually** — manage it via
`agent-toolkit project` commands instead.

```bash
# Add a repo → projects.yaml updates automatically on next devcompanion run
agent-toolkit project clone owner/my-repo
```

### .workspace.yaml

Local workspace config (gitignored). Copy from example:

```bash
cp .workspace.yaml.example .workspace.yaml
vim .workspace.yaml
```

### .gitignore

Pre-configured to ignore:

- `.workspace.yaml` — local config
- `projects.yaml` — auto-generated project index (managed by `agent-toolkit devcompanion`)
- `.active-persona` / `.active-pack` / `.active-profile` — session state
- `loops/*/STATE.md` + `loops/*/runs/` — loop runtime state
- `.agent-toolkit/` — swarm runtime state
- `repos/` — cloned repositories
- `projects/` — symlinks
- Environment files (`.env`)
- IDE files (`.vscode/`, `.idea/`)
- Token/credential files

### What is version-controlled vs gitignored

| Path | Tracked | Notes |
|------|---------|-------|
| `AGENTS.md`, `docs/`, `schemas/`, `templates/` | ✅ committed | Workspace contract and shared state |
| `knowledge/` (learnings, processes, skills) | ✅ committed | Shared memory — review before pushing |
| `knowledge/todos/pending.md` | ✅ committed (auto-generated) | Derived from devcompanion queue; regenerated via `sync-todos` |
| `packs/*.yaml` (examples) | ✅ committed | Sanitized examples only — client packs with secrets stay gitignored |
| `personas/*.md` | ✅ committed | Workspace persona selections |
| `profiles/*.yaml` | ✅ committed | Session profiles |
| `.workspace.yaml`, `projects.yaml` | ❌ gitignored | Machine-local |
| `.active-*`, `.persona-history` | ❌ gitignored | Session state |
| `loops/*/STATE.md`, `loops/*/runs/` | ❌ gitignored | Loop runtime state |
| `.agent-toolkit/` | ❌ gitignored | Swarm runtime state |
| `repos/`, `projects/` | ❌ gitignored | Local clones/symlinks |

---

## Directory Structure

After setup:

```text
agentic-harness/
├── .gitignore
├── AGENTS.md                  ← Main AI instructions
├── CLAUDE.md                  ← Symlink → AGENTS.md
├── GEMINI.md                  ← Symlink → AGENTS.md
├── README.md
├── projects.yaml          ← Auto-generated by agent-toolkit devcompanion (gitignored)
├── bin/
│   └── workspace-context      ← Session snapshot wrapper (delegates to agent-toolkit workspace context)
├── knowledge/                 ← Persistent AI memory (tracked; pending.md auto-generated)
│   ├── processes/             ← Tool and workflow docs
│   ├── learnings/             ← Discoveries and patterns
│   ├── todos/                 ← Deferred work (auto-generated from queue)
│   └── skills/                ← Skill discoveries
├── personas/                  ← Workspace persona selections (from Toolkit catalog)
├── packs/                     ← Workspace context bundles (Toolkit packs are separate)
├── profiles/                  ← Session profiles (pack + persona)
├── docs/                      ← This documentation
├── schemas/                   ← JSON Schema for context surfaces
├── templates/jobs/            ← Job templates
├── .agent-toolkit/            ← Swarm runtime state (gitignored)
├── loops/                     ← Loop state (STATE.md/runs gitignored)
├── projects/                  ← Symlinks to repos (gitignored)
└── repos/                     ← Cloned repos (gitignored)
    └── github.com/
        └── owner/
            └── my-repo/
```

---

## AI Tool Configuration

### OpenCode (Recommended)

OpenCode natively supports `AGENTS.md` and has a built-in skills system. No additional configuration needed — just open opencode in the workspace directory.

### Claude Code

Claude Code automatically reads `AGENTS.md`. No additional config needed.

Optional `~/.claude/settings.json`:

```json
{
  "permissions": {
    "allow": ["Read", "Bash", "Edit", "Write"]
  }
}
```

### Gemini CLI

Gemini CLI reads `GEMINI.md` (symlinked to `AGENTS.md`).

### GitHub Copilot

The `.github/copilot-instructions.md` symlink points to `AGENTS.md` automatically.

### Cursor

Cursor reads `CLAUDE.md` (symlinked to `AGENTS.md`).

---

## Verifying Your Setup

```bash
# Check indexed projects
agent-toolkit project list

# Check knowledge base
agent-toolkit memory todo

# Get session context snapshot
agent-toolkit workspace context

# Queue a test job (no LLM)
agent-toolkit devcompanion queue my-project --template code-review
agent-toolkit devcompanion run-once --no-llm
```

---

## Updating

```bash
cd ~/.agentic-harness
git pull origin main
```

---

## Troubleshooting

### Scripts not executable

```bash
chmod +x ./bin/*
```

### jq not found

```bash
# macOS
brew install jq

# Ubuntu/Debian
sudo apt install jq

# Fedora/RHEL
sudo dnf install jq
```

### Repos not found

```bash
# Check what's cloned
agent-toolkit project scan

# Clone missing repos
agent-toolkit project clone owner/repo-name
```

### DevCompanion fails with "No LLM provider"

Set at least one LLM provider:

```bash
export ANTHROPIC_API_KEY="sk-ant-..."
# or
export OPENAI_API_KEY="sk-..."
```
